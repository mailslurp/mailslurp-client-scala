...
import com.mailslurp.modelspackageVersion&#x3D;17.5.0.ReplyForSms
import com.mailslurp.modelspackageVersion&#x3D;17.5.0.SentSmsDto
import com.mailslurp.modelspackageVersion&#x3D;17.5.0.SmsDto
import com.mailslurp.modelspackageVersion&#x3D;17.5.0.SmsReplyOptions
...
class SmsControllerApi(baseUrl: String) {
   ...
   */
  def getAllSmsMessages(apiKey: String)(phoneNumber: Option[UUID] = None, page: Option[Int] = None, size: Option[Int] = None, sort: Option[String] = None, since: Option[OffsetDateTime] = None, before: Option[OffsetDateTime] = None, search: Option[String] = None, favourite: Option[Boolean] = None, include: Seq[UUID]
): Request[Either[ResponseError[Exception], PageSmsProjection], Nothing] =
    basicRequest
      .method(Method.GET, uri"$baseUrl/sms?phoneNumber=${ phoneNumber }&page=${ page }&size=${ size }&sort=${ sort }&since=${ since }&before=${ before }&search=${ search }&favourite=${ favourite }&include=${ include }")
      .contentType("application/json")
      .header("x-api-key", apiKey)
   ...
   */
  def getSmsCodes(apiKey: String)(smsId: UUID, extractCodesOptions: Option[ExtractCodesOptions] = None
): Request[Either[ResponseError[Exception], ExtractCodesResult], Nothing] =
    basicRequest
      .method(Method.POST, uri"$baseUrl/sms/${smsId}/codes")
      .contentType("application/json")
      .header("x-api-key", apiKey)
   ...
   * @param phoneNumberId 
   */
  def markAllSmsAsRead(apiKey: String)(read: Option[Boolean] = None, phoneNumberId: Option[UUID] = None
): Request[Either[ResponseError[Exception], Unit], Nothing] =
    basicRequest
      .method(Method.PATCH, uri"$baseUrl/sms/read?read=${ read }&phoneNumberId=${ phoneNumberId }")
      .contentType("application/json")
      .header("x-api-key", apiKey)
      .response(asJson[Unit])
   ...
   * @param read 
   */
  def markSmsAsRead(apiKey: String)(smsId: UUID, read: Option[Boolean] = None
): Request[Either[ResponseError[Exception], SmsDto], Nothing] =
    basicRequest
      .method(Method.PATCH, uri"$baseUrl/sms/${smsId}/read?read=${ read }")
      .contentType("application/json")
      .header("x-api-key", apiKey)
      .response(asJson[SmsDto])
