...
import com.mailslurp.modelspackageVersion&#x3D;15.16.17.SimpleSendEmailOptions
...
class CommonActionsControllerApi(baseUrl: String) {
   ...
   */
  def createNewEmailAddress(apiKey: String)(allowTeamAccess: Option[Boolean] = None, useDomainPool: Option[Boolean] = None, expiresAt: Option[OffsetDateTime] = None, expiresIn: Option[Long] = None, emailAddress: Option[String] = None, inboxType: Option[String] = None, description: Option[String] = None, name: Option[String] = None, tags: Seq[String], favourite: Option[Boolean] = None, virtualInbox: Option[Boolean] = None, useShortAddress: Option[Boolean] = None
): Request[Either[ResponseError[Exception], InboxDto], Nothing] =
    basicRequest
      .method(Method.POST, uri"$baseUrl/newEmailAddress?allowTeamAccess=${ allowTeamAccess }&useDomainPool=${ useDomainPool }&expiresAt=${ expiresAt }&expiresIn=${ expiresIn }&emailAddress=${ emailAddress }&inboxType=${ inboxType }&description=${ description }&name=${ name }&tags=${ tags }&favourite=${ favourite }&virtualInbox=${ virtualInbox }&useShortAddress=${ useShortAddress }")
