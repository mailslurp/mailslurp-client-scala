...
import com.mailslurp.modelspackageVersion&#x3D;15.17.38.SimpleSendEmailOptions
...
class CommonActionsControllerApi(baseUrl: String) {
   ...
   */
  def sendEmailQuery(apiKey: String)(to: String, senderId: Option[UUID] = None, body: Option[String] = None, subject: Option[String] = None
): Request[Either[ResponseError[Exception], Unit], Nothing] =
    basicRequest
      .method(Method.POST, uri"$baseUrl/sendEmailQuery?senderId=${ senderId }&to=${ to }&body=${ body }&subject=${ subject }")
      .contentType("application/json")
      .header("x-api-key", apiKey)
      .response(asJson[Unit])
