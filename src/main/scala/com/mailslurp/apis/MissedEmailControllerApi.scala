...
import com.mailslurp.modelspackageVersion&#x3D;12.6.0.MissedEmail
import com.mailslurp.modelspackageVersion&#x3D;12.6.0.PageMissedEmailProjection
...
class MissedEmailControllerApi(baseUrl: String) {
   ...
   */
  def waitForNthMissedEmail(apiKey: String)(inboxId: UUID, timeout: Long, index: Option[Int] = None
): Request[Either[ResponseError[Exception], MissedEmail], Nothing] =
    basicRequest
      .method(Method.GET, uri"$baseUrl/missed-emails/waitForNthMissedEmail?index=${ index }")
      .contentType("application/json")
      .header("x-api-key", apiKey)
      .response(asJson[MissedEmail])
